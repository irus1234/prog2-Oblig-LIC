...
public class ProcessManagerImpl implements ProcessManager{
    ...
    private void cargarProcesos(String processCsvPath) throws IOException {
        ...
        while ((linea = br.readLine()) != null) {
            ...
            if (numeroLinea == 1 && linea.toLowerCase().startsWith("pid;")) {
                continue;
            }

            /*
             * Se usa split(";", 4) para separar solo las 4 columnas esperadas:
             * pid, uid, name y events.
             * Esto evita problemas si el campo events llegara a contener algún ';'.
             */
            ...
            if (partes.length != 4) {
                ...
                throw new IllegalArgumentException(
                        "Formato inválido en process.csv línea " + numeroLinea
                );
            }
            ...
            try {
                ...
                br.close();
                throw new IllegalArgumentException(
                        "PID inválido en process.csv línea " + numeroLinea
                );
            }
            ...
            try {
                ...
                br.close();
                throw new IllegalArgumentException(
                        "UID inválido en process.csv línea " + numeroLinea
                );
            }
            ...
            if (procesosPorPid.contains(pid)) {
                br.close();
                throw new IllegalArgumentException(
                        "PID duplicado en process.csv línea " + numeroLinea + ": " + pid
                );
            }
            ...
            if (usuario == null) {
                br.close();
                throw new IllegalArgumentException(
                        "Usuario inexistente para proceso en process.csv línea "
                                + numeroLinea + ". UID: " + uid
                );
            }
            ...
            /*
             * Según el diseño:
             * - El proceso entra en estado NEW.
             * - Se guarda en la cola procesosNew.
             * - Se indexa por PID para búsquedas rápidas.
             * - Se agrega a procesosEnMemoria para consultas generales.
             */
    ...
    private MyList<Evento> parsearEventos(String eventosRaw, int numeroLinea) {
        if (eventosRaw == null || eventosRaw.trim().isEmpty()) {
            throw new IllegalArgumentException(
                    "Eventos vacíos en process.csv línea " + numeroLinea
            );
        }
        ...
        if (!eventosRaw.startsWith("{") || !eventosRaw.endsWith("}")) {
            throw new IllegalArgumentException(
                    "Formato de eventos inválido en process.csv línea " + numeroLinea
            );
        }
        ...
        if (contenido.isEmpty()) {
            throw new IllegalArgumentException(
                    "Proceso sin eventos en process.csv línea " + numeroLinea
            );
        }
        ...
        for (String eventoRaw : eventosSeparados) {
            ...
            if (posicionDosPuntos == -1
                    || posicionCorcheteInicio == -1
                    || posicionCorcheteFin == -1
                    || posicionCorcheteInicio < posicionDosPuntos
                    || posicionCorcheteFin < posicionCorcheteInicio) {

                throw new IllegalArgumentException(
                        "Evento mal formado en process.csv línea " + numeroLinea
                );
            }
            ...
            try {
                ...
                throw new IllegalArgumentException(
                        "Tipo de evento inválido en process.csv línea "
                                + numeroLinea + ": " + tipoTexto
                );
            }

            String instruccionesRaw = eventoRaw
                    .substring(posicionCorcheteInicio + 1, posicionCorcheteFin)
                    .trim();

            if (instruccionesRaw.isEmpty()) {
                throw new IllegalArgumentException(
                        "Evento sin instrucciones en process.csv línea " + numeroLinea
                );
            }

            MyList<String> instrucciones = new MyLinkedListImpl<>();

            String[] instruccionesSeparadas = instruccionesRaw.split(",");
        ...
        if (eventos.isEmpty()) {
            throw new IllegalArgumentException(
                    "Proceso sin eventos válidos en process.csv línea " + numeroLinea
            );
        }
    ...
    private void cargarUsuarios(String usersCsvPath) throws IOException {
        ...
        while ((linea = br.readLine()) != null) {
            ...
            if (numeroLinea == 1 && linea.toLowerCase().startsWith("uid;")) {
                continue;
            }
            ...
            if (partes.length != 3) {
                br.close();
                throw new IllegalArgumentException(
                        "Formato inválido en users.csv línea " + numeroLinea
                );
            }
            ...
            try {
                ...
                br.close();
                throw new IllegalArgumentException(
                        "UID inválido en users.csv línea " + numeroLinea
                );
            }
            ...
            try {
                ...
                br.close();
                throw new IllegalArgumentException(
                        "Tipo de usuario inválido en users.csv línea " + numeroLinea
                );
            }
            ...
            if (usuariosPorUid.contains(uid)) {
                br.close();
                throw new IllegalArgumentException(
                        "UID duplicado en users.csv línea " + numeroLinea + ": " + uid
                );
            }

            Usuario usuario = new Usuario(uid, alias, tipo);

            usuariosPorUid.put(uid, usuario);
